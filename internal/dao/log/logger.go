...
// New 新建实例
// 使用示例： xlog.S(ctx).InfoW(), xlog.S(ctx).Warnf() , xlog.S(ctx).Error() 等，
// 级别 debug info warn error 等都是常规级别
// 方法名 无后缀、f后缀、w后缀 三种后缀使用方法略有不同，可以具体查看定义
func New(conf Config) (*zap.Logger, error) {
	...
	_, _ = zap.RedirectStdLogAt(logger, conf.level())
...
func getBaseCore(conf Config) zapcore.Core {
	...
	return zapcore.NewCore(
		encoderFromFormat(conf.Format, conf.LevelColor), // 编码器配置
...
func getErrorCore(conf Config) zapcore.Core {
	if conf.File.Filename != "" {
		...
		encoder := encoderFromFormat("plain", true)
		return zapcore.NewCore(
			encoder, // 编码器配置
